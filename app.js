...
app.get("/author/:par", async (req, res) => {
  try {
    ...
    const user = await Author.find({ author_name: { $eq: req.params.par } })
      .lean()
      .exec();
    return res.status(201).send({ user });
  } catch (e) {
...
app.get("/author/:id/author_name", async (req, res) => {
  try {
    ...
    const user2 = await Author.find((user_ids = user.user_id))
      .lean()
      .exec();
...
app.get("/books/:par", async (req, res) => {
  try {
    console.log(req.params.par);
    const user = await Book.find({ author_name: { $eq: req.params.par } })
      .lean()
      .exec();
    return res.status(201).send(user);
  } catch (e) {
...
app.get("/section/:par", async (req, res) => {
  try {
    ...
    const user = await Section.find({
      $or: [
        { status: { $eq: req.params.par } },
        { author_name: { $eq: req.params.par } },
      ],
    })
      .lean()
      .exec();
